...
class SentimentAnalyzer():
    ...
    def setUpVocab (self, positive_doc, negative_doc):
        ...
        for line in lines:
            ...
            self.all_label_list.append("T")
            s = self.sid.polarity_scores(sentence.strip("\n"))
            if s['pos'] > s['neg']:
                self.sentiment_list.append(1)
            else:
    ...
    def svmTest (self, tes_file, out_file):
        ...
        for line in lines:
            ...
            s = self.sid.polarity_scores(sent.strip("\n"))
            if s['pos'] > s['neg']:
                self.test_sentiment_list.append(1)
            else:
                self.test_sentiment_list.append(0)
    ...
    def trainSVM(self):
        ...
        #sent_vector = cv.fit_transform (self.sentiment_list)
        combined_train_vector = sp.sparse.hstack((train_vectors, sp.sparse.csr_matrix(self.sentiment_list).T)) 
        
        
        test_vectors = self.vectorizer.transform(self.test_sent_list)
        #test_sent_vector = cv.transform(self.test_sentiment_list) 
        combined_test_vector = sp.sparse.hstack((test_vectors, sp.sparse.csr_matrix(self.test_sentiment_list).T))

        classifier = svm.SVC(kernel='linear')
        #classifier.fit(train_vectors, self.all_label_list)
        classifier.fit(combined_train_vector, self.all_label_list)
        #prediction = classifier.predict(test_vectors)
        prediction = classifier.predict(combined_test_vector)
        print prediction
        return prediction
...
sentAnalyzer.svmTest("hotelDeceptionTest.txt", "martinTestNew.txt")
